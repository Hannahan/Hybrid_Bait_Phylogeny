...
bowtie2 --local --score-min $score -x ~/bowtie_index/Ceiba_baits -1 $fwd_p  -2 $rev_p  -U $un_p  -S $sam 2>$bowtie
samtools view -bS $sam | samtools sort - $sorted
samtools index $index
samtools mpileup -E -uf ~/bowtie_index/Ceiba_baits.fna  $index > $pileup
