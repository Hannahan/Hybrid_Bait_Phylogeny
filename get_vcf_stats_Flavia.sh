...
#Number of reads:
grep -c "Gorai" $vcf >> $output

#average Qual of reads:
grep  "Gorai" $vcf | awk 'BEGIN {max=0} {sum+=$6; if ($6>max) {max=$6}} END {print sum/NR "," max}' >> $output

#average Qual of non-varient
grep  "Gorai" $vcf | awk '{ if ($5 == ".") print $0 }' |awk 'BEGIN {max=0} {sum+=$6; if ($6>max) {max=$6}} END {print sum/NR "," max}' >> $output

#average Qual of variant:
grep  "Gorai" $vcf | awk '{ if ($5 != ".") print $0 }' |awk 'BEGIN {max=0} {sum+=$6; if ($6>max) {max=$6}} END {print sum/NR "," max}' >> $output

#Number of variants
grep  "Gorai" $vcf | awk '{ if ($5 != ".") print $0 }'| wc -l >> $output
