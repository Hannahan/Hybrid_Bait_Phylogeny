...
strict=~/iROD/vcf_for_lyndsey/strict_ambig/${acc}_strict.fna
summary=~/iROD/vcf_for_lyndsey/strict_ambig/${acc}_summary.txt

#Remove the locus names in the headers

sed "s/_consensus\.fast_$acc//g" $input > $fasta

mafft --auto --thread 4 $fasta > $mafft
