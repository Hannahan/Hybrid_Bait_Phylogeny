...
bowtie2 --local -x ~/bowtie_index/All_loci -1 f_paired.fq.gz  -2 r_paired.fq.gz  -U f_unpaired.fq.gz,r_unpaired.fq.gz  -S output.sam 2> $bowtie
...
grep -v "INDEL" output.vcf | awk '{if ($6 >= 10) print $0}' > clean.vcf
